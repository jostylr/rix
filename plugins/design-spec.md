...
| `.calculus` | Implemented pure-RiX Phase 1 and exact Phase 2 calculus | Portable abstract functions and expression graphs, public bidirectional `{#}` conversion, a semantic-ID registry, obligation-bearing higher/multivariate differentiation, and provenance-recording evaluation through linked exact/certified/approximate implementations. |
| `.analysis` | Implemented pure-RiX through Phase 2 | Calculus-backed function sequences; effective scalar limits, exact geometric series, limsup/liminf and Cauchy witnesses; obligation-bearing exchanges; and optional Cauchy/Numerics adapters that keep unsupported or sampled claims unknown. |
...
| Exact calculus expansion | `.calculus` | General-power branch policies, integration, and differential/integral-equation specifications beyond the implemented registry, linked evaluator, obligation records, and higher/multivariate exact differentiator. |
| Mathematical analysis expansion | `.analysis` | Function spaces, operator analysis, and broader theorem providers beyond the implemented Phase 2 effective-limit, series, Cauchy, and justified-exchange slice. |
