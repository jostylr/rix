...
class Parser {
  ...
  isCallableNode(node) {
    ...
    if (node.type === "MethodLift") return true;
    ...
    switch (token.type) {
      ...
      case "Symbol":
        ...
        if (token.value === "?") {
          ...
          // Prefix method lift: ..Method or ..Method(args...) creates a
          // receiver-first callable. Infix obj.. remains the meta-map form.
          this.advance();
          if (this.current.type !== "Identifier") {
          ...
          if (this.current.value === "(") {
            ...
            if (this.current.value !== ")") this.error("Expected closing parenthesis in method lift");
  ...
  parseArray() {
    ...
    // Check if this might be a matrix/shaped by looking for semicolons
  ...
  parseMatrixOrArray(startToken) {
    ...
    if (this.current.value !== "]") {
      do {
        ...
        if (element.type === "BinaryOperation" && element.operator === ":=") {
          if (hasSemicolons) {
            this.error(
              "Cannot mix Shaped syntax with metadata - use nested array syntax",
            );
          }
          hasMetadata = true;
    ...
    // If we found semicolons, create Shaped node
    if (hasSemicolons) {
  ...
  buildShaped(shapedStructure, startToken) {
    const maxSeparatorLevel = Math.max(
      ...shapedStructure.map((item) => item.separatorLevel),
    );
  ...
  // `|x|` is language syntax for the internal ABS operation.  A bare pipe is
  // otherwise only meaningful as a separator after an expression (or as part
  // of a brace/generator sigil), so accepting it here in prefix position does
  // not steal the existing pipe forms.
  parseAbsoluteValue() {
    const startToken = this.current;
    ...
    return this.createNode("FunctionCall", {
      ...
      // As with operator-brace forms, ABS is generated by language syntax;
      // it is not a user-visible system capability.
      fromBrace: true,
      ...
      if (options.compactShapedType && typeName === null && this.current.type === "Identifier") {
        ...
        if (this.current.value === ":") {
          ...
          while (this.current.value !== "/" && this.current.type !== "End") {
            ...
            if (dual) this.advance();
            slots.push({ displayName, bindingName, dual });
            if (this.current.value === "@") {
              this.advance();
              continue;
            }
            if (this.current.type === "OuterIdentifier") continue;
            break;
          }
        }
        continue;
  ...
  parseBraceSigil(sigil, containerName = null, options = {}) {
    ...
    const effectiveSigil =
      isShapedShapeSigil && options.destructureAlias ? "{.." : sigil;
    ...
    return this.createNode(nodeType, {
      ...
      ...(isShapedShapeSigil && options.destructureAlias
        ? { shapedShape: containerName.split("x").map((part) => Number(part)) }
        : {}),
  ...
  parseShapedLiteral(startToken, headerText) {
    ...
    if (size === 0) {
      if (this.current.value !== "}") {
        this.error(
          `Shaped literal shape ${shape.join("x")} has size 0 and must not contain elements`,
        );
      }
    } else if (this.current.value !== "}") {
    ...
    if (level.separatorCount === 0) {
      ...
      for (let i = 0; i < level.size; i++) {
        ...
        if (i < level.size - 1) {
          if (this.current.value !== ",") {
            this.error(
              `Shaped literal shape ${shape.join("x")} expects ${level.size} columns per row`,
            );
          }
          this.advance();
        }
    ...
    for (let i = 0; i < level.size; i++) {
      ...
      if (i < level.size - 1) {
        ...
        if (consumed !== level.separatorCount) {
          ...
          this.error(
            `Shaped literal shape ${shape.join("x")} expects '${sepText}' between ${level.label}s`,
          );
        }
      }
    }
  ...
  parseEmbeddedLanguage(token) {
    ...
    if (content.startsWith(".")) {
      ...
      for (let index = 1; index < content.length; index++) {
        ...
        if (character === "(") depth++;
          ...
          depth--;
          if (depth < 0) this.error("Unmatched ')' in backtick parser header");
    ...
    // `a:b` is structural interval notation rather than a legacy LANG header.
    return this.createNode("EmbeddedLanguage", {
      language: "SArith",
      context: null,
      body: content,
