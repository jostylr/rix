...
const maxDigits=10000,maxTerms=1024;
...
function budget(value) {
    if (value instanceof RationalInterval) {rationalBudget(value.low);rationalBudget(value.high);return value;}
    if (!isExactValue(value)) {rationalBudget(value);return value;}
    const checkGenerator=generator=> {
        if (generator.polynomial?.length>64) throw new Error('Mathematical evaluation polynomial budget exceeded');
        for (const c of generator.polynomial || []) rationalBudget(c);
    };
    if (value.type==='exact_generator') checkGenerator(value);
    else {
        if (size(value)>maxTerms) throw new Error('Mathematical evaluation exact-term budget exceeded');
        for (const term of value.terms.values()) {
            rationalBudget(term.coefficient);
            if (term.powers.size>64) throw new Error('Mathematical evaluation generator budget exceeded');
            for (const [generator,power] of term.powers) {
                checkGenerator(generator);
                if (!Number.isSafeInteger(power) || Math.abs(power)>10000) throw new Error('Mathematical evaluation degree budget exceeded');
            }
        }
    }
    return value;
...
export function createProviderEvaluation(reasons) {
    ...
    function read(value) {
        ...
        const result=budget(asRational(value) || value);
    ...
    function multiply(a,b) {
        if (size(a)*size(b)>maxTerms) throw new Error('Mathematical evaluation exact-term budget exceeded');
    ...
    function operate(op,args) {
        ...
        if (op==='power') {
            ...
            if (!exponent || exponent.denominator!==1n || exponent.numerator>256n || exponent.numerator< -256n) return unsupported('unsupportedExponent');
            ...
            if (range) {
                ...
                if ([range.low,range.high].some(v=>Math.max(v.numerator.toString().length,v.denominator.toString().length)*magnitude>maxDigits)) throw new Error('Mathematical evaluation integer budget exceeded');
                return budget(a instanceof RationalInterval ? a.pow(n) : asRational(a).pow(n));
            }
        ...
        if (op==='negate') return budget(negateScalar(a));
