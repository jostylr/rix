...
1.23[56:67];     ## appended bounds: 1.2356:1.2367
1.23[+5:-6];     ## last-place offsets: 1.17:1.28
1.2[+-0.1];      ## symmetric offset: 1.19:1.21
```
...
## Certified uncertainty stays exact

```{.rix parse=true}
23.456?;          ## certified prefix; enclosure 23.456:23.457
23.456?789;       ## 789 is provisional, not part of the guarantee
3.~7~15?;         ## a continued-fraction cylinder, not an exact finite CF
(1/7).ToDecimalApproximation(5); ## 0.14285?
(1/97) ~> ".12";                ## certified bounded conversion
```

Every such value carries exact rational endpoints. Arithmetic propagates the
enclosure, while comparisons may produce the undecided decision `?`:

```{.rix parse=true}
decision := 23.456? < 23.4565;
decision ?: "yes" ?_ "no" ?? "needs refinement";
```

Here `1` is truth, `_` is a decided negative/null result, and `?` means the
current information does not decide the question. It is not treated as truth
by filters, guards, assertions, cases, or loops.

Unsigned bracket values append digits to a decimal. Signed values are offsets
measured in units of the base's last visible digit: `1.23` uses hundredths,
while `1.2` uses tenths. Thus `1.23[+5:-6]` adds `5 × 0.01` and subtracts
`6 × 0.01`; `1.2[+-0.1]` applies `0.1 × 0.1 = 0.01` in both directions.
...
`?-` is a soft preparation gate: failure means “this function does not accept that candidate.” `?!-` is strict and propagates failure. `??-` additionally lets undecided fall through; `??!-` throws unless the guard is decided and successful. Ordered variants combine into multifunctions, letting dispatch be written in RiX rather than hidden in host code.

For an expected rejection that deserves a result, use `check ?_> diagnostic`
on decided negative/null, or `check ??> diagnostic` on undecided. These return
from the active function call, including from prep or a nested block, rather
than selecting another variant. They are errors outside a call. See
[diagnostic guards and early returns](eval/function-returns.md).
