...
The `real` node uses the bounded inert-envelope loader of the existing
...
checked exactly. Opaque adapters initially write zero work, empty evidence and a null recipe.
Standalone imported envelopes may retain nonzero historical work, embedded
evidence and recipes. Loading never replays them; explicit version-matched
refinement is described in [safe interchange](../eval/safe-interchange.md).
