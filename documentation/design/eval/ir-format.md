...
| `GUARD_RETURN` | `c ?_> result` / `c ??> result` | `[{decision: "null" or "undecided"}, DEFER(condition), DEFER(value)]` | Lazy function-scoped early return; otherwise preserves the condition value; errors outside an active call |
