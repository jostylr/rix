...
::: {.callout-note title="Parser-level reference"}
This page documents the parser's broad AST vocabulary and includes historical
node names retained for design context. When exact object shape matters, the
current parser implementation and parser tests are authoritative. Runtime
support for a parsed form is separately tracked on the
[implementation status](../status.qmd) page.
:::
