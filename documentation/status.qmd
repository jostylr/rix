...
| Exact plugin implementations | `.fraction`, `.radix`, `.poly`, `.ratfun`, `.algebra`, `.exactAlgebras`, `.oracle`, `.ball`, `.cauchy`, `.continuedFraction`, `.algebraicReal`, `.numerics`, and the `.symbolic` meta-plugin are implemented in RiX; host-backed Phase 1 `.linalg`, `.optimize`, and `.solve` add exact dense systems, standard-form LP, coordinate-aware tensors, and affine symbolic solving while their service protocols settle |
...
| Packaging | the standalone manifest targets `@ratmath/core` 0.3 and its release gate covers tests, runnable examples, plugin tutorials, documentation examples, editor policy, package contents, and an isolated tarball install. That final smoke currently fails because published Core lacks APIs required by RiX; the public npm name `rix` also belongs to an unrelated project. Both block publication. |
