...
| Mathematical systems and async follow-up | `{# ... }` preserves definitions, constraints, symbols, and advisory input/output roles; `.solve` Phase 1 handles exact affine equalities and delegates matrix work to `.linalg`, while general polynomial, inequality, and nonlinear solving remain later plugin phases. Async finite collections, matrices/tensors, nested scope caps, finite pipe barriers, worker execution, cooperative top-level cancellation, and evaluator step/time budgets are implemented. Lazy bounded pull, snapshot/COW task writes, stable task paths, capability-specific abort propagation, and in-process memory accounting remain follow-up work. |
